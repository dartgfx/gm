...
T _map2F<T>(T a, T b, double Function(double, double) op) => switch (a) {
  ...
  vec3 va => vec3(op(va.x, (b as vec3).x), op(va.y, b.y), op(va.z, b.z)) as T,
  vec2 va => vec2(op(va.x, (b as vec2).x), op(va.y, b.y)) as T,
  vec4 va =>
    vec4(op(va.x, (b as vec4).x), op(va.y, b.y), op(va.z, b.z), op(va.w, b.w))
        as T,
...
T saturate<T>(T x) => switch (x) {
  ...
  vec4 v =>
    vec4(
        ...
        as T,
  _ => throw UnsupportedError('saturate: unsupported type ${x.runtimeType}'),
};
...
T smoothstep<T>(T edge0, T edge1, T x) => switch (x) {
  ...
  _ => throw UnsupportedError('smoothstep: unsupported type ${x.runtimeType}'),
};
...
T select<T>(T f, T t, dynamic cond) {
  ...
  return switch (cond) {
    vec2b c => switch (f) {
      vec2 f => vec2(c.x ? (t as vec2).x : f.x, c.y ? (t as vec2).y : f.y) as T,
      vec2i f =>
        vec2i(c.x ? (t as vec2i).x : f.x, c.y ? (t as vec2i).y : f.y) as T,
      vec2u f =>
        vec2u(c.x ? (t as vec2u).x : f.x, c.y ? (t as vec2u).y : f.y) as T,
      _ => throw UnsupportedError('select: unsupported type ${f.runtimeType}'),
    },
    vec3b c => switch (f) {
      vec3 f =>
        vec3(
...
vecb lessThan(vec a, vec b) => switch (a) {
  ...
  _ => throw UnsupportedError('lessThan: unsupported type ${a.runtimeType}'),
};
...
vecb lessThanEqual(vec a, vec b) => switch (a) {
  ...
  vec4u a => vec4b(a.x <= (b as vec4u).x, a.y <= b.y, a.z <= b.z, a.w <= b.w),
  _ =>
    throw UnsupportedError('lessThanEqual: unsupported type ${a.runtimeType}'),
};
...
vecb greaterThan(vec a, vec b) => switch (a) {
  ...
  vec3u a => vec3b(a.x > (b as vec3u).x, a.y > b.y, a.z > b.z),
  vec4u a => vec4b(a.x > (b as vec4u).x, a.y > b.y, a.z > b.z, a.w > b.w),
  _ => throw UnsupportedError('greaterThan: unsupported type ${a.runtimeType}'),
};
...
vecb equal(vec a, vec b) => switch (a) {
  ...
  vec4b a => vec4b(a.x == (b as vec4b).x, a.y == b.y, a.z == b.z, a.w == b.w),
};
